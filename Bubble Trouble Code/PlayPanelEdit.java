...
public class PlayPanel extends JPanel{
  ...
  private JButton redhome,greenhome,yellowhome,bluehome,playbutton,selected;
  ...
  private JPanel makeCenterPanel () {
   ...
   //Sets the upper right quadrent to green
   for(int i=6;i<11;i++){
     for(int j=0;j<5;j++){
   ...
   //Sets the lower left quadrent to red
   for(int i=0;i<5;i++){
     for(int j=6;j<11;j++){
      ...
      //while (!game.isWinner()) {
          ...
          if(current.getRolled()){
            if (current.noMoves()) {
              ...
              selected= (JButton) event.getSource();
              if(getBoardPosition(selected)!=-2){//it is on the board 
               //remove icon from the current position
              ...
              else if(event.getSource() == redhome || event.getSource() == greenhome || event.getSource() == yellowhome || event.getSource() == bluehome){
...
//        //disable the die, so that the player cann't roll again 
//        buttons[5][5].setEnabled(false);
//        //enable the board and the player's pawns so that they can make a move 
//        boardEnabled(false);
//        pawnEnabled(current);
//        }
//           
        ...
        JButton selected = (JButton) event.getSource();//while the selection is not valid, show user the message and 
                                             //allow them to click another button 
           if(!isValidSelection(current,selected)){
             //JOptionPane.showMessageDialog(null,"Not a valid selection. Please choose again");
             selected = (JButton) event.getSource();
             repaint();
           }
           else{//it is valid 
             if(getBoardPosition(selected)!=-2){//it is on the board 
               ...
               game.move(current, game.getBoard().getBoard()[getBoardPosition(selected)]);
               //add icon to updated position
               selected.setIcon(chooseBoardImage(current.toString(),getBoardPosition(selected)));
             }
             else{//valid but not on the board --> must be in home 
               //problem of how to call move() which takes in a pawn when you don't know which pawns are
               //in home 
               if(current.getPawnArray()[0].getPosition()==-1){//in home 
                 game.move(current,current.getPawnArray()[0]);
               }
               else if(current.getPawnArray()[1].getPosition()==-1){//in home 
                 game.move(current,current.getPawnArray()[1]);
               }
               else if(current.getPawnArray()[2].getPosition()==-1){//in home 
                 game.move(current,current.getPawnArray()[2]);
               }
               else if(current.getPawnArray()[3].getPosition()==-1){//in home 
                 game.move(current,current.getPawnArray()[3]);
               }
