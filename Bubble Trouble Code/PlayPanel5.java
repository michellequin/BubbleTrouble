//*************************************************
//  Isabel D'Alessandro initial
//  Troubleshooting: Isabel, Michelle, Leslie
//  CS230 Final Project
//  PlayPanel3.java
//
...
public class PlayPanel extends JPanel{
  ...
  private JLabel playersturn,turn,actionbutton,action,homename,die1,die2,die3,die4,die5,die6,
    dieroll,dicon,diespacer,rolledA, rank;
  ...
  public PlayPanel(PlayGame g){
    ...
    //the roll icons and instructions
  ...
  //****************************************************************************
  //  updateBoard updates the board to display the pawns in the appropriate
  //  locatiion. It is used after every move.
  //****************************************************************************
 ...
 */
  ...
  public String getColorZone(int boardindex){
    ...
    //shouldn't happen as all board positions are between 0-39
    return "";
  }
  ...
  public boolean isValidSelection(Player p,JButton button){
    if(getBoardPosition(button)!=-2){ //if the button is part of the board
      if (game.getBoard().getBoard()[getBoardPosition(button)]==null){ //if no pawn present
        return false;
      }
      else{ //otherwise, check to see that the color of the pawn matches the player, if not return false
    ...
    //All homezones are considered valid selections if it is your color and you have a pawn in there
 ...
 public ImageIcon chooseBoardImage(String color,int boardindex){
  ...
  private class ButtonListener implements ActionListener{
    ...
    public void actionPerformed(ActionEvent event){
      ...
      updateBoard(); //keep the board up to date with all the changes
      ...
      //If there are no players initialized, display an error
      if(game.order.size()==0){
        if (event.getSource()==redhome || event.getSource()==yellowhome || event.getSource()==bluehome 
              || event.getSource()==greenhome || event.getSource()==playbutton){
        ...
        if(started){ //if the start button was clicked
            current=game.order.first();
            turn.setText("<html><font color=#FFD700 size=6>"+current.getName()+ "</font></html>");
            action.setText(roll);
          } 
        
        if(event.getSource()==buttons[5][5]){ //if they click on the die
          ...
          if (current.noMoves()){ //if the player cannot make any moves, check everything
              System.out.println("No pawns in play and did not roll a 1: " + 
                                 (current.inPlay() == 0 && current.getDieNumber() != 1));//TEST------------------
              System.out.println("No pawns in play and did not roll a 6: " + 
                                 (current.inPlay() == 0 && current.getDieNumber() != 6));//TEST------------------
              System.out.println("The player has a move: " + !current.noMoves());//TEST--------------------------
              //System.out.println("queue: " + game.order);
          ...
          else { //if the player has a possible move
            ...
            System.out.println ("At line 819 - player has a move");//TEST------------------------------
            ...
            if (selected==redhome || selected==yellowhome || selected==bluehome || selected==greenhome) {
              ...
              if (current.getDieNumber() == 6 || current.getDieNumber() == 1) {
                System.out.println("here - possible move since rolled a 1 or a 6 and clicked on home");
              ...
              System.out.println("selected a pawn that will be moved"); //TEST-------------------
              ...
              updateBoard();
              System.out.println("Moving on to next player since player has moved. Line 889");
              game.nextPlayer(current);
            }
          }
          revalidate();
